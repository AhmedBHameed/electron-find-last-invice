...
app.whenReady().then(() => {
  ...
  /**
   * Start finding highest file number
   */
  ipcMain.on("find-highest-file-num", async (event, data) => {
    try {
      const fileNames = readdirSync(data.srcPath, { withFileTypes: true })
        .filter((item) => !item.isDirectory())
        .map((item) => item.name.replace(/\.[^/.]+$/, ""));

      mainWindow.webContents.send("highest-file-num-result", {
        disableButton: false,
        fileNames: fileNames,
      });
    } catch (error) {
